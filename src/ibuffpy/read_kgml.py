...
# download pathway data from KGML
def get_pathway_kgml(pathway_id, savepath):
    # download pathway data from KGML API
    api = 'http://rest.kegg.jp/get/'+pathway_id+'/kgml'
    # save the downloaded file
    kgml = requests.get(api).text
...
def complete_reaction(reaction):
    ...
    for line in form_data.splitlines():
        ...
        raise ValueError('api data is not correct')
    ...
    kegg_lhs_cpds_formula = []
    kegg_rhs_cpds_formula = []
    for cpd in kegg_lhs_cpds:
        if cpd[0]=='C':
            kegg_lhs_cpds_formula.append(cpd)
        # when the first character is integer, it is a coefficient
        elif cpd[0].isdigit():
            coef = int(cpd[:cpd.find('C')])
            kegg_lhs_cpds_formula += [cpd[cpd.find('C'):]]*coef
        else:
            raise ValueError('cpd name is not correct')
    for cpd in kegg_rhs_cpds:
        if cpd[0]=='C':
            kegg_rhs_cpds_formula.append(cpd)
        # when the first character is integer, it is a coefficient
        elif cpd[0].isdigit():
            coef = int(cpd[:cpd.find('C')])
            kegg_rhs_cpds_formula += [cpd[cpd.find('C'):]]*coef
        else:
            raise ValueError('cpd name is not correct')
    
    # direction...  True: => False: <+
    if (not set(lhs).isdisjoint(set(kegg_lhs_cpds_formula))) and (not set(rhs).isdisjoint(set(kegg_rhs_cpds_formula))):
        direction=True
    elif (not set(lhs).isdisjoint(set(kegg_rhs_cpds_formula))) and (not set(rhs).isdisjoint(set(kegg_lhs_cpds_formula))):
        direction=False
    else:
        raise ValueError('direction cannot be determined')
    if direction:
        reaction_complete=[reaction[0],kegg_lhs_cpds_formula,kegg_rhs_cpds_formula]
    else:
        reaction_complete=[reaction[0],kegg_rhs_cpds_formula,kegg_lhs_cpds_formula]
