...
class ReactionNetwork:
    ...
    def __init__(self,reaction_list,info=True):
        ...
        if len(reaction_list[0])==3:
            ...
            self.reaction_list_noid=[[reac[1],reac[2]] for reac in reaction_list]
        elif len(reaction_list[0]==2):
            self.reaction_list=[[i,reaction_list[i]] for i in range(len(reaction_list))]
        ...
        self.reaction_id=[reac[0] for reac in self.reaction_list]

        stoi=self.make_stoi()
        self.stoi=stoi

        ns=linalg.null_space(stoi)
        ns2=linalg.null_space(stoi.T).T
        self.ns=ns
        self.ns2=ns2
        ...
        def random_limitset():
            ...
            for reac in reaction_list:
                ...
                while True:
                    ...
                    eff_r2=[reac for reac in choose_r(eff_m) if (reac not in eff_r)]
    ...
    def compute_smat_mean(self, N=10):
        ...
        np_mean_check=np.where((smat_mean<1.0e-8) & (smat_mean>1.0e-10), 1,0)
